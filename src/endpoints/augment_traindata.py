...
@click.option("--output_dir", type=click.Path(exists=False), help='output directory')
def augment_traindata(image_dir: str, gt_dir: str, sample_size: int, crop_size: int, output_dir: str):
...
def _random_crop(image_dir: Path, gt_dir: Path, filename: str, sample_size: int, crop_size: int, output_dir: Path):
